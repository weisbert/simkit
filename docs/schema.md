...
| `templatesDir` | path | no | `./templates` | Directory holding formula-template sidecar files (`<name>.template.json`). Relative paths resolved from the `.pvtproject` file's own directory. Phase 3B addition; see `docs/phase3b_measure_template_spec.md`. Additive — no `schema_version` bump. |
| `signalGroupsDir` | path | no | `./signal_groups` | Directory holding signal-group sidecar files (`<name>.siggroup.json`). Relative paths resolved from the `.pvtproject` file's own directory. Phase 3B addition. Additive — no `schema_version` bump. |
| `measurementsDir` | path | no | `./measurements` | Directory holding measurement-bundle sidecar files (`<name>.measure.json`). Relative paths resolved from the `.pvtproject` file's own directory. Phase 3B addition. Additive — no `schema_version` bump. |
| `cornerModelsDir` | path | no | `./corner_models` | Directory holding corner-manager model sidecar files (`<name>.cornermodel.json`). Relative paths resolved from the `.pvtproject` file's own directory. Phase 5 addition; see `docs/phase5_stage1_spec.md`. Additive — no `schema_version` bump. |
| `cornerLibsDir` | path | no | `./corner_libs` | Directory holding reusable corner-library sidecar files (`<name>.cornerlib.json`). Relative paths resolved from the `.pvtproject` file's own directory. Phase 5 Stage 5 addition; see `docs/phase5_stage5_spec.md`. Additive — no `schema_version` bump. |
| `pvtProfilesDir` | path | no | `./pvt_profiles` | Directory holding PVT-profile sidecar files (`<name>.pvtprofile.json`) — the per-project semantic→concrete PVT map. Relative paths resolved from the `.pvtproject` file's own directory. Phase 5 Stage 6 addition; see `docs/phase5_stage6_spec.md`. Additive — no `schema_version` bump. |
