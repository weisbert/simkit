...
- [x] Copy simulated netlist to run dir — fixed 2026-05-12 (DECISIONS #27). Simulator probe was the wrong API for Maestro; replaced with the path-presence check that was already gating the copy (only Spectre writes `input.scs`). Tier-2 verified on `simkit_verify`: `netlist_path="input.scs"`, 2938-byte file copied, `pvt validate` clean.
