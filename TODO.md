...
- [ ] **2 pre-existing SKILL Tier-1 FAILs** — surfaced during runTests.il fix verification. (a) `corners/collectRowNames/skips-missing-row_name` (added in v1.9 #1, was passing then; may be a Cadence-state-dependent flake); (b) `fresh path under existing dir accepted` (dialog fixture cleanup race in `/tmp/dialog_…`). Neither blocks v1.9 #3 ship; investigate next session.
