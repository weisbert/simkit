...
**UPDATE 2026-05-18 (later same day): the "post-run focus loss" hypothesis above is WRONG and is retracted. Real root cause + true fix in DECISIONS #60.** Briefly: it wasn't focus stealing — it was a skillbridge socket-steal by axl worker VMs. Worker virtuoso processes (spawned by `axlRunAllTests` for sim dispatch) load `.cdsinit` → `sbStart.il` → `pyStartServer`, which spawned a SECOND python_server bound to the same `/tmp/skill-server-default.sock`. The Python client then routed to the worker's server, which had no Maestro session registered. Patching `sbStart.il` to detect "I am an axl worker" via `/proc/$PPID/cmdline` containing `-axlChildIdFlag` and skip `pyStartServer` in that case eliminates the steal. Validated 2026-05-18: full v1.4 dogfood (snapshot → pick → pre-run install → axlRunAllTests → PvtSave → ingest → corner restore) completes with **zero user-click recovery required**. The v1.4.1 polymorphism followup in this section's earlier paragraph is therefore **no longer needed** — `Snapshot/RestoreCornersEnable` can stay name-based since name resolution survives axl dispatch once the worker stops stealing the socket.

## #60 — Skillbridge `sbStart.il` skips `pyStartServer` in axl worker VMs
_Date: 2026-05-18_

**Decision:** patch `<workarea>/skill_tools/skillbridge/sbStart.il` to detect axl worker virtuoso processes and skip the `pyStartServer` call in their VMs. Worker detection: shell out to `grep -aq axlChildIdFlag /proc/$PPID/cmdline && echo y` via `ipcBeginProcess` (the `$PPID` inside the spawned shell resolves to the SKILL virtuoso). Main VM (no `-axlChildIdFlag` arg) goes through the original startup path; worker VMs print a "skipping pyStartServer" notice and load the SKILL defs without starting a Python server.

**Why:** every `axlRunAllTests` fan-out spawns one or more worker virtuoso processes (`runICRP<N>` → `virtuoso ... -axlChildIdFlag <N>`). Each worker loads `.cdsinit`, which loads `sbStart.il`, which unconditionally fires `pyStartServer` — binding `/tmp/skill-server-default.sock`. The worker's server takes over the socket binding (last-bind-wins for UNIX domain sockets), so the next Python client `Workspace.open()` connects to the worker's SKILL VM instead of the main VM. The worker has no Maestro Assembler session registered → `axlGetMainSetupDB("fnxSession0")` returns "Cannot find an active session". This was misdiagnosed for two days as "post-run window focus loss" (DECISIONS #58 + #59 v1.4.1 note) and worked around by asking the user to click Maestro back into focus — which "fixed" things only because the worker's python_server happened to die around the same time, freeing the socket back to the main VM.

Root cause discovered 2026-05-18 by tracing the orphan python_server pair's parent chain (`ps -o pid,ppid,cmd`): orphan parent = a `virtuoso -axlChildIdFlag 16` process, not the main Virtuoso. Verified by inspecting the orphan's worker log file: contained `[skillbridge] pyStartServer requested ...` print line emitted by `sbStart.il`'s startup block.

**Alternatives considered (all rejected):**
- *Use a per-VM socket name (e.g. `worker-<pid>` in workers, `default` in main)*: requires either env-var coordination between main and workers (worker has no way to know what name main used), or unique socket per process (then Python clients can't auto-discover). Patching sbStart to skip is simpler.
- *Kill workers' python_servers after they spawn*: race-y, requires polling, adds complexity.
- *Move sbStart.il invocation out of `.cdsinit` into a Maestro-only init hook*: would have to find that hook; `.cdsinit` is the well-known entry point, breaking convention is worse than the targeted skip.
- *Probe via `getShellEnvVar` for a worker-only env var*: spawn args (`-axlChildIdFlag <N>`) don't propagate to env vars; no reliable env-var marker exists.

**Implementation gotchas surfaced during the patch (do not re-walk these — pinned for future SKILL coders):**
1. **`procedure( name() ...)` requires NO space between name and `()`** — `procedure( name ()...)` errors with `illegal formal list`. Already in DECISIONS #14 idiom list trap #13; I violated my own rule.
2. **`if(...)` paren form is strict 3-arg Lisp-style** — does NOT accept `then`/`else` keywords inside parens. Use `if(cond progn(then-stmts...) progn(else-stmts...))` for multi-statement branches.
3. **`getc(port)` returns a SYMBOL not a string** — `stringp(c)` is nil, `symbolp(c)` is t, `(type c)` returns `'symbol`. `charToInt` works on the char-symbol.
4. **`getc` returns the symbol `t` for BOTH EOF and for the character 't'** (ASCII 116) — they are indistinguishable. Any char-by-char reader of binary content using `neq(c t)` as the EOF check will truncate at the first 't' byte. There is no `eofp` / `feof` predicate on this Cadence. Workaround: read via shell pipe (`ipcBeginProcess` + `ipcReadProcess`), or use `grep`/`sed` to do the parsing entirely in shell.
5. **`/proc/self/cmdline` from SKILL refers to the SKILL VM itself; from a spawned shell via `ipcBeginProcess`, `/proc/self` is the shell, but `$PPID` is the SKILL VM** — use `$PPID` when reading parent process state from shelled-out commands.

**Live-verified 2026-05-18 on `fnxSession0`**: ran the v1.4 dogfood end-to-end (snapshot → pick auto-baseline TT → install pre-run → axlRunAllTests → PvtSave → ingest → restore enable state) in a single Python script invocation with NO user click required at any point. `ps` shows pair count stays constant across `axlRunAllTests` (no new orphan spawned). Worker's Job log confirms the new printf: `[skillbridge] axl worker VM detected (cmdline has -axlChildIdFlag); skipping pyStartServer`.

**Patch lives in `<workarea>/skill_tools/skillbridge/sbStart.il`** (a sibling repo to simkit, separately versioned). Commit there. Simkit memory updated: [[reference_skillbridge_recovery]] should be edited to note that the multi-pair stacking trap is the AXL-worker-socket-steal pattern, not a wedge — and that the proper fix is the sbStart.il patch, not pkill + re-fire.

## #61 — Phase 3A v1.5: rdb-walker `pvtRunnerCountRunning` as primary run-completion discriminator
_Date: 2026-05-18_

**Decision:** add SKILL `pvtRunnerCountRunning(sess)` that opens the current history's rdb and counts rows whose `tst->status` is NOT in the terminal set `{done, failed, no_convergence, aborted, killed}`. Wire Python's `pvt_runner_run` state machine to AND this signal with `axlGetRunStatus`: a run is "done" only when BOTH read idle for `idle_confirm_reads` consecutive polls. Existing kwargs preserved; new opt-in kwarg `min_running_observed: int = 0` lets the orchestrator demand "saw at least N concurrent in-flight rows before believing the dispatch grace" for slow-queue cases.

**Why:** 2026-05-18 PM Phase 3A v1.5 dogfood (first real `pvt run` via the newly-wired CLI, against `~/cadence_work/simkit_p3b_dogfood/reviews/fnxsession0_v1dog.review.json`, 2 items × 6 sub-points each) caught the long-known v1.2 #1 leftover *in production*: for item 1 (`Test`), `axlGetRunStatus` returned `(0, 0)` from the very first poll — the state machine exited via `dispatch_grace_reads` BEFORE Maestro had queued any sub-point. PvtSave then dumped 6 `__sim_status__` rows all with status='pending' (real sub-points never ran). Item 2 (`Test_trans`) caught 3 sub-points done + 1 running + 2 pending at PvtSave time — Spectre had started, but PvtSave fired before it finished. Both runs landed in DB only after F1 (validator) was permissive enough to accept the `pending` rows, but the rows themselves were unusable for analysis. The rdb-walker signal is independent of `axlGetRunStatus`'s blind-spot behaviour and gives the state machine ground truth on per-sub-point completion.

**Mechanism:** the rdb (results database) maintained by Maestro during a run has one row per (test, point) with a `tst->status` symbol. The collector's `_pvtCollWalkRdb` (Phase 1 §3 messy-data refactor) already proves rdb walking is safe during the async tail (DECISIONS #54 #4). `pvtRunnerCountRunning` reuses the same path: `axlGetCurrentHistory` → `maeReadResDB` on the rdb pathname → walk `(rdb->tests)` → predicate-filter on `_pvtRunnerIsPendingStatus(tst->status)`. Edge cases all return 0 (no current history, rdb path missing, `maeReadResDB` returns nil) — translates "no live work to report" into a clean idle signal, never raises.

**Alternatives considered (all rejected):**
- *Replace `axlGetRunStatus` entirely with the rdb signal*: would lose the cheap fast-path for cached runs and the early-error case where axlGetRunStatus DOES signal failure. AND-ing keeps both signals' value.
- *Poll `axlGetCurrentHistory` for a "completed" / "error" slot*: previous probes (DECISIONS #50/#53) found no such slot on this Cadence; the rdb is the only structured source of per-sub-point status.
- *`wait_for_worker_exit_sec` polling `ps -p <axl-worker-pid>`*: was floated as the v1.2 #1 alternative (a). Requires a SKILL helper to expose the worker PID and adds shell-out overhead per poll. The rdb walker stays inside SKILL and gives finer-grained data (per-sub-point status, not just "process alive").
- *Treat post-rename cleanup as best-effort with exponential-backoff retry*: was v1.2 #1 alternative (c). Pragmatic but pushes the problem to runtime and doesn't help PvtSave fire at the right moment.

**Live-verified 2026-05-18 on `fnxSession0`**: 2 rounds of full `pvt run` on the same v1dog review file, both rounds 2 items completed cleanly. Round 1 item 1 = 36 rows (6 outputs × 6 sub-points) with 30 ok + 6 eval_err (no pending, no running); round 1 item 2 = 42 rows (7 outputs × 6 sub-points) with 6 ok + 36 eval_err. Round 2 byte-identical (cached Spectre, ~16s gap between item ingests). Contrast with pre-F2 run: item 1 had 6 pending sentinels (all sub-points never started). Restored corner + test enable state byte-identical in both rounds.

**Implementation notes:**
- Status classification helper `_pvtRunnerIsPendingStatus(sym)` treats anything OTHER than `{done, failed, no_convergence, aborted, killed}` as in-flight (including `'running`, `'pending`, AND unknown symbols). Defensive: a Cadence version that adds a new status symbol won't accidentally exit the poll early.
- The Python state machine's `saw_running` flag was added to track count-side observation separately from `saw_non_idle` (axlGetRunStatus side); the gate is `saw_non_idle OR saw_running`, and idle requires BOTH signals consecutive-idle.
- `_pvtRunnerCountRunning` returns `pvt_ok`/`pvt_err` envelope per existing pattern.
